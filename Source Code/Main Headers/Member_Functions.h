...
//======================= Storing Infos  =================================
        ...
        //1-A random which will be our "Encryption_Key "is generated every time the user try to input his/her Password
...
int  Modify_Or_Delete_Member(char Delete_Modify,Member_credentials *ADHERANT_CREDENTIALS,Member_infos *ADHERANT_INFOS)
{
        ...
        if (strcmp(ADHERANT_CREDENTIALS->Member_Status,"ADMIN") == 0)
        {
                ...
                if (Delete_Modify == 'D') {
                        ...
                        Infos_Input(ADHERANT_INFOS);
                        CHANGE_OR_DELETE_DATABASE_LINE('M',ADHERANT_CREDENTIALS,pSOURCE_DATABASE,pDest_DATABASE, Login_ID);
                ...
                if (Delete_Modify == 'D') {
                        ...
                        Infos_Input(ADHERANT_INFOS);
                        CHANGE_OR_DELETE_DATABASE_LINE('M',&Temp,pSOURCE_DATABASE,pDest_DATABASE,ADHERANT_CREDENTIALS->Member_ID);
...
void Member_Alpha_Display(){
        ...
        for (int i = 0; i < Member_Number; i++) {
                printf("\n\n\t \t############################ Member %d ######################################\n\n",i+1);
                printf("\t\t\tMember Login : %s     | Status : %s \n",test[i].Member_Login,test[i].Member_Status);
